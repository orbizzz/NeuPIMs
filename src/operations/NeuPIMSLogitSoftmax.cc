...
uint32_t NeuPIMSLogitSoftmax::sram_size_needed() {
    ...
    // params_per_head *= _config.precision;

    // uint32_t available_sram = _config.spad_size KB / 2;  // double buffering
    // assert(params_per_head < available_sram);
    // // if assertion fail, divide tiling batch dimension too.
    // uint32_t head_coverage = floor((double)available_sram / params_per_head);
    // spdlog::info("available sram: {}B", available_sram);
    // spdlog::info("params per head: {}B", params_per_head);
    // spdlog::info("heads per NPU tile: {}", head_coverage);
    // exit(-1);
    return 0;  // need_size;
}