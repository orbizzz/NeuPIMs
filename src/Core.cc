...
void Core::running_log() {
    auto top_tile = _tiles.front();
    spdlog::info("------Simulator Status Check------");
    spdlog::info("_core_cycle: {}", _core_cycle);
    spdlog::info("num_tiles: {}", _tiles.size());
    if (_tiles.size() > 0) {
        spdlog::info("remain_load: {}, remain_store: {} of top_tile", top_tile->remaining_loads,
                     top_tile->remaining_accum_io);
    }
    spdlog::info("_compute_pipeline: {}", !_compute_pipeline.empty());
    spdlog::info("_waiting_write_reqs: {}", _waiting_write_reqs != 0);
    spdlog::info("_ld_inst_queue: {}", !_ld_inst_queue.empty());
    spdlog::info("_st_inst_queue: {}", !_st_inst_queue.empty());
    spdlog::info("_ex_inst_queue: {}", !_ex_inst_queue.empty());
    int vector_idx = 0;
    for (auto &vector_pipeline : _vector_pipelines) {
        spdlog::info("vp#{} size: {}", vector_idx, vector_pipeline.size());
        vector_idx++;
    }
}
bool Core::running() {
    ...
    int status_check_interval = 100000;
