...
std::vector<MemoryAccess *> MemoryAccess::from_instruction(Instruction &inst, uint32_t id,
    ...
    const addr_type max_address =
        Config::global_config.model_n_embd * Config::global_config.model_n_embd * 5 * 2;
...
void initialize_system_config(std::string sys_config_path) {
    ...
    if (sys_config["sub_batch_mode"]) assert(Config::global_config.dram_type == DramType::NEUPIMS);
